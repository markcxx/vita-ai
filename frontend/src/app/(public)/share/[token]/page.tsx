  ...
  const fetchResume = useCallback(async (pwd?: string) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        if (data.passwordRequired || data.detail?.passwordRequired) {
          if (pwd) {
  ...
  if (notFound) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950">
        ...
        <Link href="/" className="mt-4 text-sm text-brand hover:text-brand">
          返回 {appName}
        </Link>
      </div>
    );
  ...
  if (needsPassword) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950">
        ...
        <Link href="/" className="mt-6 text-sm text-brand hover:text-brand">
          返回 {appName}
        </Link>
      </div>
    );
      ...
      <div className="min-h-screen bg-zinc-100 dark:bg-zinc-950">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between gap-2 border-b border-zinc-200 bg-white px-4 py-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex min-w-0 items-center gap-2">
            <span className="shrink-0 text-sm font-semibold text-brand">{appName}</span>
