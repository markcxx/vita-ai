import { legacyAccentCSS } from '@/lib/template-palette';
...
export type ResumeWithSections = Omit<Resume, 'themeConfig'> & {
...
export function degreeField(degree: string | undefined, field: string | undefined): string {
...
export function buildQrCodesHtml(section: Section): string {
  const c = section.content as QrCodesContent;
...
export function buildExportThemeCSS(theme: ThemeConfig, template: string): string {
  ...
  return `
    ...
    ${legacyAccentCSS(sel, theme.accentColor)}
  `;
}
