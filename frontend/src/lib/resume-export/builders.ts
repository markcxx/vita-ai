...
import { recolorExportHtml } from '@/lib/template-palette';
...
import type { QrCodesContent } from '@/types/resume';
...
async function preGenerateQrSvgs(resume: ResumeWithSections): Promise<void> {
  ...
  const qrContent = qrSection.content as QrCodesContent;
...
export async function generateHtml(resume: ResumeWithSections, forPdf = false): Promise<string> {
  resume = { ...resume, sections: withDefaultAvatars(resume.sections) };
  // Pre-generate QR SVGs so sync template builders can use them
  await preGenerateQrSvgs(resume);
  if (isReferenceTemplate(resume.template)) {
    const markup = renderToStaticMarkup(createElement(ReferenceTemplate, { resume: { ...resume, themeConfig: resume.themeConfig || undefined } as import('@/types/resume').Resume }));
    return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(resume.title)}</title><style>
      *{box-sizing:border-box}body{margin:0;background:white;font-family:Arial,"Noto Sans CJK SC",sans-serif}
      .resume-export{width:794px;margin:auto}img{max-width:100%}h2{break-after:avoid}li,[data-resume-entry]{break-inside:avoid}
      @page{size:A4;margin:0}@media print{.resume-export{width:210mm}.resume-export>div{overflow:visible!important;min-height:0!important}}
      *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
    </style></head><body><div class="resume-export">${markup}</div></body></html>`;
  }
  ...
  return recolorExportHtml(`<!DOCTYPE html>
...
<body>
  <div class="resume-export" data-avatar-style="${esc(resume.themeConfig?.avatarStyle || 'circle')}">
    ${bodyHtml}
  </div>
</body>
</html>`, theme.accentColor);
}
