...
function buildStartupSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="pl-4" style="border-left-width:3px;border-left-style:solid;border-color:${CYAN}">
  ...
  if (section.type === 'skills') {
    return `<div class="flex flex-wrap gap-1.5">${((c as SkillsContent).categories || []).flatMap((cat) =>
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-4">${((c as ProjectsContent).items || []).map((it) => `<div class="pl-4" style="border-left-width:3px;border-left-style:solid;border-color:${CYAN}">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-1.5">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-4">${((c as GitHubContent).items || []).map((it) => `<div class="pl-4" style="border-left-width:3px;border-left-style:solid;border-color:${CYAN}">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-4">${((c as CustomContent).items || []).map((it) => `<div class="pl-4" style="border-left-width:3px;border-left-style:solid;border-color:${CYAN}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-medium text-zinc-800">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
