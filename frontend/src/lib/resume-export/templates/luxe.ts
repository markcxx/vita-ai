...
function buildLuxeSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-5">${((c as WorkExperienceContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-4">${((c as EducationContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-4">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-4">${((c as GitHubContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-4">${((c as CustomContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-bold" style="color:${TEXT}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm" style="color:#44403c">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
