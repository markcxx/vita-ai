...
function buildZigzagSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (c.items) {
    return `<div class="space-y-2 text-left">${c.items.map((it) => `<div><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
