...
function buildJapaneseSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-6">${((c as WorkExperienceContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'education') {
    return `<div class="space-y-5">${((c as EducationContent).items || []).map((it) => `<div>
      <div class="flex items-baseline justify-between">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-5">${((c as ProjectsContent).items || []).map((it) => `<div>
      <div class="flex items-baseline justify-between">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-x-8 gap-y-2">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-5">${((c as GitHubContent).items || []).map((it) => `<div>
      <div class="flex items-baseline justify-between">
        <h3 class="text-sm font-normal" style="color:${PRIMARY}">${esc(it.name)}</h3>
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-4">${((c as CustomContent).items || []).map((it) => `<div>
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-normal" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm font-light" style="color:#57534e">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
