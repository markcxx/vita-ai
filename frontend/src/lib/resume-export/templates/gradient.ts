...
function buildGradientSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1">
  ...
  if (section.type === 'certifications') {
    return `<div class="space-y-1.5">${((c as CertificationsContent).items || []).map((it) =>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="border-l-2 pl-4" style="border-image:${GRADIENT};border-image-slice:1"><span class="text-sm font-medium" style="color:${ACCENT}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${esc(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
