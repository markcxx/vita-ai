...
function buildMinimalSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-medium text-zinc-700">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-500">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
