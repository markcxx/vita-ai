...
function buildArtisticSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="relative rounded-lg p-4" style="border:1px dashed ${ACCENT}30">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="rounded-lg p-4" style="border:1px dashed ${ACCENT}30">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="rounded-lg p-4" style="border:1px dashed ${ACCENT}30">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="rounded-lg p-4" style="border:1px dashed ${ACCENT}30">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="rounded-lg p-4" style="border:1px dashed ${ACCENT}30">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="rounded-lg p-3" style="border:1px dashed ${ACCENT}30"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
