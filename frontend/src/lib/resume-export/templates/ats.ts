...
function buildAtsSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'education') {
    return `<div class="space-y-2">${((c as EducationContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'languages') {
    return `<p class="text-sm text-zinc-700">${((c as LanguagesContent).items || []).map((it, i: number, arr) =>
  ...
  if (c.items) {
    return `<div class="space-y-1">${c.items.map((it) => `<div><span class="text-sm font-bold text-black">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-700">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
