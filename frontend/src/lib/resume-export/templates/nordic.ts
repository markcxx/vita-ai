...
function buildNordicSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="rounded-sm p-3" style="background-color:${SLATE_50}">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="rounded-sm p-3" style="background-color:${SLATE_50}">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="rounded-sm p-3" style="background-color:${SLATE_50}">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="rounded-sm p-3" style="background-color:${SLATE_50}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-medium" style="color:${SLATE_500}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm font-light" style="color:${SLATE_400}">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
