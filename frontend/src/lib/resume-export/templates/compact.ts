...
function buildCompactRightContent(section: Section, lang: string): string {
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-2">${((c as ProjectsContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-2">${((c as GitHubContent).items || []).map((it) => `<div>
  ...
  if (c.items) {
    return `<div class="space-y-1.5">${c.items.map((it) => `<div>
...
function buildCompactLeftContent(section: Section): string {
  ...
  if (section.type === 'languages') {
    return `<div class="space-y-0.5">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-1.5">${((c as CustomContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'qr_codes') return buildQrCodesHtml(section);
  if (c.items) {
    return `<div class="space-y-1">${c.items.map((it) => `<div>
