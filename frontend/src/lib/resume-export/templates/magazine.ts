...
function buildMagazineSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
  ...
  if (section.type === 'skills') {
    return `<div class="grid grid-cols-2 gap-x-6 gap-y-3">${((c as SkillsContent).categories || []).map((cat) =>
  ...
  if (section.type === 'projects') {
    return `<div class="grid grid-cols-2 gap-4">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-4">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}"><span class="text-sm font-bold" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm" style="color:${SECONDARY}">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
