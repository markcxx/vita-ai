...
function buildCleanSectionContent(s: Section, lang: string): string {
  ...
  if (s.type === 'work_experience') {
    return `<div class="space-y-4">${(c.items || []).map((it) => `<div>
  ...
  if (s.type === 'education') {
    return `<div class="space-y-3">${(c.items || []).map((it) => `<div>
      <div class="flex items-baseline justify-between">
  ...
  if (s.type === 'skills') {
    return `<div class="flex flex-wrap gap-2">${(c.categories || []).flatMap((cat) =>
