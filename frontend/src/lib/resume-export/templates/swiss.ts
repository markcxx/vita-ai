...
function buildSwissSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="grid grid-cols-[140px_1fr] gap-4">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="grid grid-cols-[140px_1fr] gap-4">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="grid grid-cols-[140px_1fr] gap-4">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-x-6 gap-y-1.5">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="grid grid-cols-[140px_1fr] gap-4">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="grid grid-cols-[140px_1fr] gap-4">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div><span class="text-sm font-bold" style="color:${TEXT}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm" style="color:#3f3f46">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
