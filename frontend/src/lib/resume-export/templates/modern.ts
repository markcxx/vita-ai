...
function buildModernSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:#e94560">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:#0f3460">
  ...
  if (section.type === 'skills') {
    const allSkills = ((c as SkillsContent).categories || []).flatMap((cat) => cat.skills || []);
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-4">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:#e94560">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-4">${((c as GitHubContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:#e94560">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:#e94560">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="border-l-2 border-zinc-200 pl-4"><span class="text-sm font-medium text-zinc-700">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
