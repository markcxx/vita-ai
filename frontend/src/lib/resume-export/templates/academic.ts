...
function buildAcademicSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-2.5">${((c as WorkExperienceContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'education') {
    return `<div class="space-y-2.5">${((c as EducationContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'skills') {
    return `<div class="space-y-0.5">${((c as SkillsContent).categories || []).map((cat) =>
      `<p class="text-sm text-zinc-600"><span class="font-bold text-zinc-700">${esc(cat.name)}: </span>${esc((cat.skills || []).join(', '))}</p>`
    ).join('')}</div>`;
  }
  if (section.type === 'projects') {
    return `<div class="space-y-2.5">${((c as ProjectsContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'certifications') {
    return `<div class="space-y-1">${((c as CertificationsContent).items || []).map((it) =>
  ...
  if (section.type === 'languages') {
    return `<p class="text-sm text-zinc-600">${((c as LanguagesContent).items || []).map((it, i: number, arr) =>
      `<span class="font-bold text-zinc-700">${esc(it.language)}</span> (${esc(it.proficiency)})${i < arr.length - 1 ? '; ' : ''}`
    ).join('')}</p>`;
  }
  if (section.type === 'github') {
    return `<div class="space-y-2.5">${((c as GitHubContent).items || []).map((it) => `<div>
  ...
  if (c.items) {
    return `<div class="space-y-1">${c.items.map((it) => `<div><span class="text-sm font-bold text-zinc-700">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
