...
function buildArchitectSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'skills') {
    return `<div class="space-y-2">${((c as SkillsContent).categories || []).map((cat) =>
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-2 pl-4" style="border-color:${ACCENT}">
