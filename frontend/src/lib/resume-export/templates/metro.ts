...
function buildMetroSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="border-l-3 pl-4" style="border-color:${AMBER}"><span class="text-sm font-semibold" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${esc(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
