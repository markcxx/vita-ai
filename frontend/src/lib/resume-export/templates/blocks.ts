...
function buildBlocksSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-3">${((c as WorkExperienceContent).items || []).map((it) => `<div class="rounded-md border p-3" style="border-color:#e3e2de">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="rounded-md border p-3" style="border-color:#e3e2de">
  ...
  if (section.type === 'skills') {
    return `<div class="overflow-hidden rounded-md border" style="border-color:#e3e2de"><table class="w-full text-sm"><tbody>${((c as SkillsContent).categories || []).map((cat) =>
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="rounded-md border p-3" style="border-color:#e3e2de">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="rounded-md border p-3" style="border-color:#e3e2de">
  ...
  if (section.type === 'certifications') {
    ...
    return `<div class="overflow-hidden rounded-md border" style="border-color:#e3e2de">${items.map((it, idx: number) =>
  ...
  if (section.type === 'languages') {
    return `<div class="overflow-hidden rounded-md border" style="border-color:#e3e2de"><table class="w-full text-sm"><tbody>${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="rounded-md border p-3" style="border-color:#e3e2de">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="rounded-md border px-3 py-2" style="border-color:#e3e2de"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm" style="color:#787774">${esc(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
