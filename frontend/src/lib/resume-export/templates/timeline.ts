import type { GitHubContent, SummaryContent } from '@/types/resume';
...
function buildTimelineSectionContent(s: Section, lang: string): string {
  ...
  if (s.type === 'summary') return `<div class="text-sm leading-relaxed text-zinc-600">${md((c as SummaryContent).text)}</div>`;

  if (s.type === 'work_experience') {
    const items = c.items || [];
    return `<div class="relative border-l-2 pl-6 ml-2" style="border-color:#e2e8f0">${items.map((it, idx: number) => `<div class="relative${idx < items.length - 1 ? ' pb-5' : ''}">
  ...
  if (s.type === 'education') {
    const items = c.items || [];
    return `<div class="relative border-l-2 pl-6 ml-2" style="border-color:#e2e8f0">${items.map((it, idx: number) => `<div class="relative${idx < items.length - 1 ? ' pb-4' : ''}">
      <div class="absolute -left-[31px] top-1 h-4 w-4 rounded-full border-2 bg-white" style="border-color:${AC}"></div>
      <div class="flex items-baseline justify-between">
  ...
  if (s.type === 'skills') {
    return `<div class="space-y-1">${(c.categories || []).map((cat) =>
  ...
  if (s.type === 'github') {
    return `<div class="relative border-l-2 pl-6 ml-2" style="border-color:#e2e8f0">${((c as GitHubContent).items || []).map((it, idx: number, arr) => `<div class="relative${idx < arr.length - 1 ? ' pb-5' : ''}">
      <div class="absolute -left-[31px] top-1 h-4 w-4 rounded-full border-2 bg-white" style="border-color:${AC}"></div>
  ...
  if (s.type === 'certifications') {
    return `<div class="space-y-1.5">${(c.items || []).map((it) =>
  ...
  if (s.type === 'languages') {
    return `<div class="flex flex-wrap gap-x-6 gap-y-1">${(c.items || []).map((it) =>
  ...
  if (s.type === 'custom') {
    return `<div class="space-y-2">${(c.items || []).map((it) => `<div>
