...
function buildSidebarSectionContent(section: Section): string {
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-1.5">${((c as CustomContent).items || []).map((it) =>
  ...
  if (c.items) {
    return `<div class="space-y-1.5">${c.items.map((it) => `<div><span class="text-xs font-medium text-blue-100">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-[10px] text-blue-300">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
...
function buildSidebarMainContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-3">${((c as WorkExperienceContent).items || []).map((it) => `<div>
  ...
  if (section.type === 'skills') {
    return `<div class="space-y-1">${((c as SkillsContent).categories || []).map((cat) =>
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-2">${((c as CustomContent).items || []).map((it) => `<div>
