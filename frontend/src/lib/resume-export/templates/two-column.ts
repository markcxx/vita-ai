...
export function buildTwoColumnLeftContent(section: Section): string {
  ...
  if (c.items) {
    return `<div class="space-y-1.5">${c.items.map((it) => `<div><span class="text-xs font-medium text-zinc-200">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-[10px] text-zinc-400">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
