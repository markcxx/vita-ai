...
function buildRoseSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-2">${((c as LanguagesContent).items || []).map((it) =>
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="rounded-xl border p-3" style="border-color:${ROSE_100}"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
