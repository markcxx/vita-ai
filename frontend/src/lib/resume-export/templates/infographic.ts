...
function buildInfographicSectionContent(section: Section, color: string, colorIndex: number, lang: string): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it) => `<div class="rounded-lg border border-zinc-100 p-4">
  ...
  if (section.type === 'skills') {
    return `<div class="space-y-2">${((c as SkillsContent).categories || []).map((cat, ci: number) => `<div>
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it) => `<div class="rounded-lg border border-zinc-100 p-4">
  ...
  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-2">${((c as LanguagesContent).items || []).map((it, i: number) =>
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it) => `<div class="rounded-lg border border-zinc-100 p-3"><span class="text-sm font-medium text-zinc-800">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
