...
export function useResume() {
  ...
  const createResume = useCallback(async (data: { title?: string; template?: string; language?: string; themeConfig?: ThemeConfig }) => {
    try {
      const res = await fetch('/api/resume', {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({ ...data, ...(data.template && isReferenceTemplate(data.template) && !data.themeConfig ? { themeConfig: referenceTheme(data.template) } : {}) }),
      });
      if (res.ok) {
