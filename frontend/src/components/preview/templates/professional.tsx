    ...
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
      ...
      <div className="mb-6 text-center">
        <div className="flex items-center justify-center gap-4">
          {pi.avatar && (
            <AvatarImage src={pi.avatar} avatarStyle={resume.themeConfig?.avatarStyle} size={72} className="shrink-0" style={{ border: '2px solid var(--resume-palette-1e3a5f, #1e3a5f)' }} />
          )}
          <div>
            <h1 className="text-3xl font-bold tracking-wide" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>
        ...
        <div className="mt-4 h-[2px] w-full" style={{ background: 'linear-gradient(90deg, transparent 0%, var(--resume-palette-1e3a5f, #1e3a5f) 20%, var(--resume-palette-1e3a5f, #1e3a5f) 80%, transparent 100%)' }} />
      </div>
        ...
        .map((section) => (
          <div key={section.id} className="mb-5" data-section>
            <div className="mb-3 flex items-center gap-3">
              <h2 className="text-sm font-bold uppercase tracking-[0.2em]" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>
                {section.title}
              </h2>
...
function ProfessionalSectionContent({ section, lang }: { section: TemplateResumeSection; lang?: string }) {
        ...
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-sm font-bold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.position}</span>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-sm font-bold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.institution}</span>
                {item.location && <span className="text-sm text-zinc-400"> ({item.location})</span>}
              </div>
      ...
      <div className="space-y-1.5">
        {categories.map((cat) => (
          <div key={cat.id} className="flex text-sm">
            <span className="w-32 shrink-0 font-semibold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{cat.name}:</span>
    ...
    return (
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.name}</span>
              {item.startDate && (
      ...
      <div className="space-y-1.5">
        {items.map((item) => (
          <div key={item.id} className="flex items-baseline justify-between text-sm">
            <div>
              <span className="font-semibold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.name}</span>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1">
        {items.map((item) => (
          <span key={item.id} className="text-sm">
            <span className="font-semibold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.language}</span>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-sm font-semibold" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.title}</span>
    ...
    return (
      <div className="space-y-2">
        {content.items.map((item) => (
          <div key={item.id}>
            <span className="text-sm font-medium" style={{ color: 'var(--resume-palette-1e3a5f, #1e3a5f)' }}>{item.name || item.title || item.language}</span>
