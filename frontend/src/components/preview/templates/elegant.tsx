...
function ElegantSectionContent({ section, lang }: { section: TemplateResumeSection; lang?: string }) {
      ...
      <div className="space-y-4">
        {(content.items || []).map((item) => (
          <div key={item.id}>
            ...
            {(item.technologies?.length ?? 0) > 0 && <p className="mt-0.5 text-xs text-zinc-400 italic">{lang === 'zh' ? '技术栈' : 'Tech'}: {item.technologies?.join(', ')}</p>}
