...
export function TwoColumnTemplate({ resume }: { resume: Resume }) {
  ...
  return (
    <div className="mx-auto flex max-w-[210mm] overflow-hidden bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif', minHeight: '297mm' }}>
      {/* Left Column */}
      <div className="w-[35%] shrink-0 p-6 text-white" style={{ background: 'linear-gradient(180deg, var(--resume-palette-1a1a2e, #1a1a2e) 0%, var(--resume-palette-16213e, #16213e) 100%)' }}>
        {/* Avatar & Name */}
      ...
      <div className="flex-1 p-6">
        {rightSections.map((section) => (
          <div key={section.id} className="mb-5" data-section>
            <h2 className="mb-2 border-b-2 pb-1 text-sm font-bold uppercase tracking-wider" style={{ color: 'var(--resume-palette-1a1a2e, #1a1a2e)', borderColor: 'var(--resume-palette-1a1a2e, #1a1a2e)' }}>
              {section.title}
            </h2>
...
function LeftSectionContent({ section }: { section: TemplateResumeSection }) {
...
function RightSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  const content = section.content;
  if (!content) return null;
