...
import type { Resume, PersonalInfoContent, SummaryContent, ProjectsContent, CertificationsContent, LanguagesContent, CustomContent, GitHubContent } from '@/types/resume';
  ...
  return (
    <div className="mx-auto max-w-[210mm] overflow-hidden bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, var(--resume-palette-1a1a2e, #1a1a2e) 0%, var(--resume-palette-16213e, #16213e) 50%, var(--resume-palette-0f3460, #0f3460) 100%)' }}
      >
        {/* Decorative circles */}
        <div
          className="absolute -right-10 -top-10 h-40 w-40 rounded-full opacity-10"
          style={{ background: 'radial-gradient(circle, var(--resume-palette-e94560, #e94560) 0%, transparent 70%)' }}
        />
        <div
        ...
        <div className="relative flex items-center gap-6">
          {pi.avatar && (
            <AvatarImage src={pi.avatar} avatarStyle={resume.themeConfig?.avatarStyle} size={80} className="border-2 border-white/10" wrapperClassName="shrink-0 p-[2px]" wrapperStyle={{ background: 'linear-gradient(135deg, var(--resume-palette-e94560, #e94560), var(--resume-palette-0f3460, #0f3460))' }} />
          )}
          <div className="min-w-0 flex-1">
            <h1 className="text-3xl font-bold tracking-tight">{pi.fullName || 'Your Name'}</h1>
            {pi.jobTitle && (
              <p className="mt-1.5 text-base font-light tracking-wide" style={{ color: 'var(--resume-palette-e94560, #e94560)' }}>
                {pi.jobTitle}
              </p>
            )}
        ...
        <div
          ...
          style={{ background: 'linear-gradient(90deg, var(--resume-palette-e94560, #e94560) 0%, var(--resume-palette-0f3460, #0f3460) 60%, transparent 100%)' }}
        />
      </div>
        ...
        {resume.sections
          ...
          .map((section) => (
            <div key={section.id} className="mb-6" data-section>
              <h2 className="mb-3 flex items-center gap-2.5 text-sm font-bold uppercase tracking-wider" style={{ color: 'var(--resume-palette-e94560, #e94560)' }}>
                <span className="h-[3px] w-7 rounded-full" style={{ background: 'linear-gradient(90deg, var(--resume-palette-e94560, #e94560), var(--resume-palette-0f3460, #0f3460))' }} />
                {section.title}
              </h2>
...
function ModernSectionContent({ section, lang }: { section: TemplateResumeSection; lang?: string }) {
    ...
    return (
      <div className="space-y-4">
        {(content.items || []).map((item) => (
          <div key={item.id} className="border-l-2 pl-4" style={{ borderColor: 'var(--resume-palette-e94560, #e94560)' }}>
            ...
            {item.company && <p className="text-sm" style={{ color: 'var(--resume-palette-e94560, #e94560)' }}>{item.company}</p>}
  ...
  if (section.type === 'education') {
    return (
      <div className="space-y-3">
        {(content.items || []).map((item) => (
          <div key={item.id} className="border-l-2 pl-4" style={{ borderColor: 'var(--resume-palette-0f3460, #0f3460)' }}>
  ...
  if (section.type === 'certifications') {
    ...
    return (
      <div className="space-y-1.5">
        {items.map((item) => (
          <div key={item.id} className="flex items-baseline justify-between border-l-2 pl-4" style={{ borderColor: 'var(--resume-palette-0f3460, #0f3460)' }}>
            <div>
