...
const PRIMARY = 'var(--resume-palette-1e293b, #1e293b)';
const ACCENT = 'var(--resume-palette-0284c7, #0284c7)';
const SECONDARY = 'var(--resume-palette-64748b, #64748b)';
const BODY_TEXT = 'var(--resume-palette-334155, #334155)';
const RULE_COLOR = 'var(--resume-palette-cbd5e1, #cbd5e1)';
...
export function EngineerTemplate({ resume }: { resume: Resume }) {
  ...
  return (
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif' }}>
      {/* Dark steel header */}
      <div className="px-8 py-6" style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, var(--resume-palette-334155, #334155) 100%)` }}>
        <div className="flex items-center gap-5">
          ...
          <div className="shrink-0 text-right">
            <div className="space-y-0.5 text-xs" style={{ color: 'var(--resume-palette-94a3b8, #94a3b8)' }}>
              {pi.age && <p>{pi.age}</p>}
...
function EngineerSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  ...
  if (section.type === 'work_experience') {
    return (
      <div className="space-y-4">
        {((content as WorkExperienceContent).items || []).map((item) => (
          <div key={item.id}>
  ...
  if (section.type === 'education') {
    return (
      <div className="space-y-3">
        {((content as EducationContent).items || []).map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
  ...
  if (section.type === 'skills') {
    return (
      <div className="space-y-2">
        {((content as SkillsContent).categories || []).map((cat) => (
          <div key={cat.id}>
            <span
  ...
  if (section.type === 'projects') {
    return (
      <div className="space-y-3">
        {((content as ProjectsContent).items || []).map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
  ...
  if (section.type === 'github') {
    return (
      <div className="space-y-3">
        {((content as GitHubContent).items || []).map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: PRIMARY }}>{item.name}</span>
  ...
  if (section.type === 'certifications') {
    return (
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item) => (
  ...
  if (section.type === 'languages') {
    return (
      <div className="space-y-1.5">
        {((content as LanguagesContent).items || []).map((item) => (
          <div key={item.id} className="flex items-center gap-2">
            <span className="h-1 w-1 shrink-0" style={{ backgroundColor: ACCENT }} />
  ...
  if (section.type === 'custom') {
    return (
      <div className="space-y-3">
        {((content as CustomContent).items || []).map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
