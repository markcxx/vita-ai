...
const BLUE_GRAY = 'var(--resume-palette-475569, #475569)';
const ACCENT = 'var(--resume-palette-3b82f6, #3b82f6)';
...
function TimelineSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
        ...
        {(content.items || []).map((item, idx: number) => (
          <div key={item.id} className={`relative ${idx < (content.items || []).length - 1 ? 'pb-5' : ''}`}>
            ...
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: 'var(--resume-palette-eff6ff, #eff6ff)', color: ACCENT }}>
            ...
            {(item.technologies?.length ?? 0) > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {item.technologies?.map((t: string, i: number) => (
                  <span key={i} className="rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: 'var(--resume-palette-eff6ff, #eff6ff)', color: ACCENT }}>
                    {t}
                  </span>
                ))}
              </div>
