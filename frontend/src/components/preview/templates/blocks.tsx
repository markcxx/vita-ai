...
const ACCENT = 'var(--resume-palette-2383e2, #2383e2)';
...
function BlocksSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
                ...
                <td className="px-3 py-2">
                  <div className="flex flex-wrap gap-1">
                    {(cat.skills || []).map((skill: string, i: number) => (
                      <span key={i} className="rounded-sm px-2 py-0.5 text-xs" style={{ backgroundColor: `color-mix(in srgb, ${ACCENT} 7.06%, transparent)`, color: ACCENT }}>
                        {skill}
                      </span>
                    ))}
                  </div>
