...
const BLUE = 'var(--resume-palette-1e40af, #1e40af)';
        ...
        {resume.sections
          ...
          .map((section) => (
            <div key={section.id} className="mb-5 flex gap-4" data-section>
              ...
              <div className="flex-1 border-l-2 pl-4" style={{ borderColor: 'var(--resume-palette-dbeafe, #dbeafe)' }}>
...
function EuroSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
      ...
      <div className="space-y-3">
        {(content.items || []).map((item) => (
          <div key={item.id}>
            ...
            {(item.technologies?.length ?? 0) > 0 && <p className="mt-0.5 text-xs text-zinc-400">{resume.language === 'zh' ? '技术栈' : 'Tech'}: {item.technologies?.join(', ')}</p>}
