...
const PRIMARY = 'var(--resume-palette-0f172a, #0f172a)';
const ACCENT = 'var(--resume-palette-0891b2, #0891b2)';
const GRID_LINE = '#e2e8f0';
const BODY_TEXT = 'var(--resume-palette-334155, #334155)';
const MUTED = 'var(--resume-palette-64748b, #64748b)';
...
function ScientistSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  ...
  if (section.type === 'work_experience') {
    return (
      <div className="space-y-4">
        {((content as WorkExperienceContent).items || []).map((item, idx: number) => (
          <div key={item.id}>
  ...
  if (section.type === 'education') {
    return (
      <div className="space-y-3">
        {((content as EducationContent).items || []).map((item, idx: number) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-xs font-bold" style={{ color: ACCENT }}>[{idx + 1}]</span>
  ...
  if (section.type === 'projects') {
    return (
      <div className="space-y-3">
        {((content as ProjectsContent).items || []).map((item, idx: number) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-xs font-bold" style={{ color: ACCENT }}>[{idx + 1}]</span>
  ...
  if (section.type === 'certifications') {
    return (
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item, idx: number) => (
  ...
  if (section.type === 'custom') {
    return (
      <div className="space-y-3">
        {((content as CustomContent).items || []).map((item, idx: number) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-xs font-bold" style={{ color: ACCENT }}>[{idx + 1}]</span>
