...
const PRIMARY = 'var(--resume-palette-1e1b4b, #1e1b4b)';
const ACCENT = 'var(--resume-palette-f43f5e, #f43f5e)';
const HIGHLIGHT = 'var(--resume-palette-fbbf24, #fbbf24)';
          ...
          .map((section) => (
            <div key={section.id} className="mb-6" data-section>
              <div className="mb-2 flex items-center gap-2">
                ...
                <div className="h-0.5 flex-1" style={{ borderTop: `2px dashed color-mix(in srgb, ${ACCENT} 25.1%, transparent)` }} />
              </div>
...
function ArtisticSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  const content = section.content;
  if (!content) return null;

  if (section.type === 'summary') {
    return (
      <div className="rounded-lg p-4" style={{ border: `2px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)`, backgroundColor: `color-mix(in srgb, ${PRIMARY} 1.96%, transparent)` }}>
  ...
  if (section.type === 'work_experience') {
    ...
    return (
      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.id} className="relative rounded-lg p-4" style={{ border: `1px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)` }}>
  ...
  if (section.type === 'education') {
    ...
    return (
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id} className="rounded-lg p-4" style={{ border: `1px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)` }}>
            <div className="flex items-baseline justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {(cat.skills || []).map((skill: string, i: number) => (
                <span
                  ...
                  style={{ backgroundColor: `color-mix(in srgb, ${PRIMARY} 6.27%, transparent)`, color: PRIMARY }}
                >
  ...
  if (section.type === 'certifications') {
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <div key={item.id} className="rounded-lg px-4 py-2" style={{ border: `1px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)`, backgroundColor: `color-mix(in srgb, ${HIGHLIGHT} 6.27%, transparent)` }}>
  ...
  if (section.type === 'languages') {
    ...
    return (
      <div className="flex flex-wrap gap-3">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-2 rounded-full px-4 py-1.5" style={{ border: `2px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)` }}>
    ...
    return (
      <div className="space-y-2">
        {content.items.map((item) => (
          <div key={item.id} className="rounded-lg p-3" style={{ border: `1px dashed color-mix(in srgb, ${ACCENT} 18.82%, transparent)` }}>
