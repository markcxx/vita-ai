...
const PRIMARY = 'var(--resume-palette-9a3412, #9a3412)';
const ACCENT = 'var(--resume-palette-ea580c, #ea580c)';
const WARM_BG = 'var(--resume-palette-fff7ed, #fff7ed)';
const BODY_TEXT = 'var(--resume-palette-374151, #374151)';
const MUTED = '#78716c';
...
function TeacherSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
        ...
        {((content as WorkExperienceContent).items || []).map((item) => (
          <div key={item.id} className="rounded-lg border-l-3 p-3" style={{ borderColor: ACCENT, backgroundColor: WARM_BG }}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ backgroundColor: 'var(--resume-palette-fed7aa, #fed7aa)', color: PRIMARY }}>
            ...
            {(item.technologies?.length ?? 0) > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {item.technologies?.map((t: string, i: number) => (
                  <span key={i} className="rounded-full px-2 py-0.5 text-[10px]" style={{ backgroundColor: 'var(--resume-palette-fed7aa, #fed7aa)', color: PRIMARY }}>
                    {t}
                  </span>
                ))}
              </div>
