...
const PRIMARY = 'var(--resume-palette-4f46e5, #4f46e5)';
const VIOLET = 'var(--resume-palette-7c3aed, #7c3aed)';
...
function MaterialSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  ...
  if (section.type === 'languages') {
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-2 rounded-full px-4 py-1.5 shadow-sm" style={{ backgroundColor: `color-mix(in srgb, ${PRIMARY} 6.27%, transparent)` }}>
