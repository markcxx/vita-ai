...
const ACCENT = 'var(--resume-palette-8b5cf6, #8b5cf6)';
const ALT_BG = '#f5f3ff';
  ...
  return (
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="mb-6 flex items-center justify-center gap-1">
        {[...Array(20)].map((_, i) => (
          <div key={i} className="h-1 w-3 rounded-full" style={{ backgroundColor: i % 2 === 0 ? ACCENT : `color-mix(in srgb, ${ACCENT} 25.1%, transparent)` }} />
        ))}
      </div>
...
function ZigzagSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
          ...
          <div key={cat.id}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(cat.skills || []).map((skill: string, i: number) => (
                <span key={i} className="rounded-full border px-2 py-0.5 text-xs font-medium" style={{ borderColor: `color-mix(in srgb, ${ACCENT} 31.37%, transparent)`, color: ACCENT, backgroundColor: `color-mix(in srgb, ${ACCENT} 3.14%, transparent)` }}>
                  {skill}
                </span>
              ))}
            </div>
  ...
  if (section.type === 'languages') {
    ...
    return (
      <div className="flex flex-wrap gap-3">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-2 rounded-full px-3 py-1" style={{ backgroundColor: `color-mix(in srgb, ${ACCENT} 6.27%, transparent)`, border: `1px solid color-mix(in srgb, ${ACCENT} 18.82%, transparent)` }}>
