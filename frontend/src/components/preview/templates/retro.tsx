...
const PRIMARY = 'var(--resume-palette-78350f, #78350f)';
const ACCENT = 'var(--resume-palette-92400e, #92400e)';
const BG = 'var(--resume-palette-fefce8, #fefce8)';
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs" style={{ color: ACCENT }}>
            {contacts.map((c, i) => (
              <span key={i} className="flex items-center gap-1.5" style={{ fontFamily: "'Courier New', monospace" }}>
                {c}
                {i < contacts.length - 1 && <span style={{ color: `color-mix(in srgb, ${PRIMARY} 25.1%, transparent)` }}>{'\u2022'}</span>}
              </span>
            ))}
          </div>
        )}
        {/* Ornamental divider */}
        <div className="mx-auto mt-3 flex items-center justify-center gap-2 text-sm" style={{ color: `color-mix(in srgb, ${PRIMARY} 37.65%, transparent)` }}>
          <span>~</span>
          <span>{'\u2666'}</span>
          <span>~</span>
        </div>
      ...
      {resume.sections
        ...
        .map((section, idx, arr) => (
          <div key={section.id} data-section>
            ...
            {idx < arr.length - 1 && (
              <div className="mb-4 flex items-center justify-center gap-2 text-xs" style={{ color: `color-mix(in srgb, ${PRIMARY} 25.1%, transparent)` }}>
                <div className="h-px w-12" style={{ backgroundColor: `color-mix(in srgb, ${PRIMARY} 12.55%, transparent)` }} />
                <span>{'\u2726'}</span>
                <div className="h-px w-12" style={{ backgroundColor: `color-mix(in srgb, ${PRIMARY} 12.55%, transparent)` }} />
              </div>
            )}
...
function RetroSectionContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
