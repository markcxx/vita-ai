...
import type { ResumeSection as TemplateResumeSection, QrCodesContent as TemplateQrCodesContent } from '@/types/resume';
...
const DARK = '#0d1117';
const BLUE = 'var(--resume-palette-58a6ff, #58a6ff)';
const GREEN = 'var(--resume-palette-3fb950, #3fb950)';
const BORDER = '#21262d';
    ...
    <div className="mx-auto flex max-w-[210mm] overflow-hidden bg-white shadow-lg" style={{ fontFamily: '"JetBrains Mono", "Fira Code", monospace', minHeight: '297mm' }}>
      ...
      <div className="w-[32%] shrink-0 p-5" style={{ backgroundColor: DARK }}>
        ...
        <div className="mb-4 flex items-center gap-1.5">
          ...
          <span className="ml-2 text-[10px]" style={{ color: 'var(--resume-palette-484f58, #484f58)' }}>~/whoami</span>
        </div>
        ...
        <div className="mb-5 space-y-1.5 text-[11px]" style={{ borderTop: `1px solid ${BORDER}`, paddingTop: '12px' }}>
          {pi.age && (
            <div className="flex items-start gap-2">
              ...
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.age}</span>
            </div>
          )}
          {pi.politicalStatus && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.politicalStatus}</span>
            </div>
          )}
          {pi.gender && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.gender}</span>
            </div>
          )}
          {pi.ethnicity && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.ethnicity}</span>
            </div>
          )}
          {pi.hometown && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.hometown}</span>
            </div>
          )}
          {pi.maritalStatus && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.maritalStatus}</span>
            </div>
          )}
          {pi.yearsOfExperience && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.yearsOfExperience}</span>
            </div>
          )}
          {pi.educationLevel && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.educationLevel}</span>
            </div>
          )}
          {pi.email && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span className="break-all" style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.email}</span>
            </div>
          )}
          {pi.phone && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.phone}</span>
            </div>
          )}
          {pi.wechat && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.wechat}</span>
            </div>
          )}
          {pi.location && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.location}</span>
            </div>
          )}
          {pi.website && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span className="break-all" style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.website}</span>
            </div>
          )}
          {pi.github && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span className="break-all" style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.github}</span>
            </div>
          )}
          {pi.linkedin && (
            <div className="flex items-start gap-2">
              <span style={{ color: GREEN }}>$</span>
              <span className="break-all" style={{ color: 'var(--resume-palette-8b949e, #8b949e)' }}>{pi.linkedin}</span>
            </div>
          )}
        </div>
...
function CoderSidebarContent({ section }: { section: TemplateResumeSection }) {
  ...
  if (section.type === 'skills') {
    ...
    return (
      <div className="space-y-2">
        {categories.map((cat) => (
          <div key={cat.id}>
            <p className="text-[10px] font-semibold" style={{ color: 'var(--resume-palette-c9d1d9, #c9d1d9)' }}>{cat.name}</p>
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between text-[10px]">
            <span style={{ color: 'var(--resume-palette-c9d1d9, #c9d1d9)' }}>{item.language}</span>
            <span style={{ color: 'var(--resume-palette-484f58, #484f58)' }}>{item.proficiency}</span>
          </div>
        ))}
      </div>
    );
  ...
  if (section.type === 'certifications') {
    ...
    return (
      <div className="space-y-1.5">
        {items.map((item) => (
          <div key={item.id}>
            <p className="text-[10px] font-semibold" style={{ color: 'var(--resume-palette-c9d1d9, #c9d1d9)' }}>{item.name}</p>
            {(item.issuer || item.date) && <p className="text-[9px]" style={{ color: 'var(--resume-palette-484f58, #484f58)' }}>{item.issuer}{item.date ? ` (${item.date})` : ''}</p>}
          </div>
        ))}
      </div>
    );
  ...
  if (section.type === 'qr_codes') {
    return <QrCodesPreview items={(content as TemplateQrCodesContent).items || []} />;
  }
    ...
    return (
      <div className="space-y-1.5">
        {content.items.map((item) => (
          <div key={item.id}>
            <span className="text-[10px] font-medium" style={{ color: 'var(--resume-palette-c9d1d9, #c9d1d9)' }}>{item.name || item.title || item.language}</span>
            {item.description && <p className="text-[9px]" style={{ color: 'var(--resume-palette-484f58, #484f58)' }} dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
          </div>
        ))}
      </div>
    );
...
function CoderMainContent({ section, resume }: { section: TemplateResumeSection; resume: Resume }) {
  const content = section.content;
  if (!content) return null;
        ...
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 rounded px-2 py-0.5 text-[10px] font-medium" style={{ backgroundColor: '#f6f8fa', color: 'var(--resume-palette-57606a, #57606a)' }}>
            ...
            {(item.technologies?.length ?? 0) > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {item.technologies?.map((t: string, i: number) => (
                  <span key={i} className="rounded-sm px-1.5 py-0.5 text-[10px] font-medium" style={{ backgroundColor: '#f6f8fa', color: 'var(--resume-palette-57606a, #57606a)', border: '1px solid #d0d7de' }}>
                    {t}
                  </span>
                ))}
              </div>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id}>
            ...
            {item.language && (
              <span className="rounded-sm px-1.5 py-0.5 text-[10px] font-medium" style={{ backgroundColor: '#f6f8fa', color: 'var(--resume-palette-57606a, #57606a)', border: '1px solid #d0d7de' }}>
                {item.language}
              </span>
            )}
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
