...
export function RadarChart({ dimensions, comparisonDimensions, comparisonLabel }: RadarChartProps) {
  ...
  const safeComparison = Array.isArray(comparisonDimensions) ? comparisonDimensions : undefined;
  const data = safeDimensions.map((d) => {
    const item: Record<string, string | number> = {
      dimension: d.dimension,
      score: d.score,
    };
    if (safeComparison) {
      const match = safeComparison.find((c) => c.dimension === d.dimension);
    ...
    <div className="rounded-lg border bg-white p-6 dark:bg-zinc-900">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <RechartsRadar data={data}>
          ...
          <Radar name={t('overallScore')} dataKey="score" stroke="#419cf5" fill="#419cf5" fillOpacity={0.3} />
          {safeComparison && (
            <Radar
