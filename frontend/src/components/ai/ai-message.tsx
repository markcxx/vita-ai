...
function isToolPart(part: UIMessage['parts'][number]): part is RenderableToolPart {
...
function ToolCallCard({ part }: { part: RenderableToolPart }) {
  const t = getCopy('ai');

  const toolName = getAIToolName(part);
  ...
  const isFailureResult = typeof result === 'object' && result !== null && 'success' in result && result.success === false;
  const isSuccess = isCompleted && !isFailureResult;
