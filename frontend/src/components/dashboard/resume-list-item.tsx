...
export function ResumeListItem({ resume, onDelete, onDuplicate, onRename }: ResumeListItemProps) {
  ...
  return (
    <div
      className={`group flex items-center gap-4 rounded-xl border border-border bg-card px-5 py-5 shadow-sm transition-all duration-200 dark:border-zinc-700/60 dark:bg-card/90 ${isRenaming ? '' : 'cursor-pointer hover:border-brand hover:shadow-md'}`}
