...
import { resumeAvatar } from '@/lib/resume/avatar-defaults';
...
import { getResumeFieldOptions } from '@/lib/resume/field-constraints';
...
export function PersonalInfoSection({ section, onUpdate }: Props) {
  ...
  const tTheme = getCopy('themeEditor');
  const content = section.content as PersonalInfoContent;
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { currentResume } = useResumeStore();
  const avatarStyle = currentResume?.themeConfig?.avatarStyle || referenceTheme(currentResume?.template || '').avatarStyle || 'circle';
  const avatar = resumeAvatar(content.avatar);
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label="更换简历头像"
          style={{ width: 80, height: avatarStyle === 'circle' ? 80 : 112, borderRadius: avatarStyle === 'circle' ? '50%' : 4 }}
        ...
        >
          {avatar ? (
            <img src={avatar} alt="简历头像" className="h-full w-full object-cover" />
          ) : (
          ...
          <div className="inline-flex rounded-lg bg-zinc-100 p-0.5 dark:bg-zinc-800">
            {([
              ...
              <button
                ...
                aria-pressed={avatarStyle === value}
      ...
      <FieldWrapper>
        ...
        <EditableSelect
          ...
          options={getResumeFieldOptions('personal_info', 'gender').map((s) => ({ label: s, value: s }))}
        />
      </FieldWrapper>
      <FieldWrapper>
        <EditableSelect
          ...
          options={getResumeFieldOptions('personal_info', 'politicalStatus').map((s) => ({ label: s, value: s }))}
        />
        <EditableSelect
          label={t('ethnicity')}
          value={content.ethnicity || ''}
          onChange={(v) => onUpdate({ ethnicity: v })}
          options={getResumeFieldOptions('personal_info', 'ethnicity').map((s) => ({ label: s, value: s }))}
        />
      </FieldWrapper>
      <FieldWrapper>
        ...
        <EditableSelect
          ...
          options={getResumeFieldOptions('personal_info', 'maritalStatus').map((s) => ({ label: s, value: s }))}
        />
      </FieldWrapper>
      <FieldWrapper>
        ...
        <EditableSelect
          ...
          options={getResumeFieldOptions('personal_info', 'educationLevel').map((s) => ({ label: s, value: s }))}
        />
      </FieldWrapper>
      <FieldWrapper>
