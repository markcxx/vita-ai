...
export function hasCompletedTour(tourId: string): boolean {
  ...
  return getPreference(storageKey(tourId)) === '1';
}
...
export const useTourStore = create<TourStore>((set, get) => ({
  ...
  nextStep: () =>
    set((s) => {
      if (s.currentStep >= s.totalSteps - 1) {
        if (s.activeTourId) setPreference(storageKey(s.activeTourId), '1');
  ...
  dismiss: () => {
    ...
    if (activeTourId) setPreference(storageKey(activeTourId), '1');
